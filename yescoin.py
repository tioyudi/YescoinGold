...
def print_welcome_message():
    print(r"""
        CCCCCCCCC HH       HH  II  KK   Kk  II
        CCCCCCCCC HH       HH  II  KK  Kk   II
        CC        HH       HH  II  KK Kk    II
        CC        HH HHHHH HH  II  KKK      II
        CC        HH       HH  II  KK Kk    II
        CCCCCCCCC HH       HH  II  KK  Kk   II
        CCCCCCCCC HH       HH  II  KK   Kk  II
          """)
    ...
    try:
        ...
        if result['code'] == 0:
            if result['data']['collectStatus']:
                ...
                print(f"{Fore.RED + Style.BRIGHT}\r[ Chest ] :No chest         ", flush=True)
                return True, 0
        else:
    ...
    while True:
        ...
        for index, token in enumerate(tokens):
            ...
            if squad_info and squad_info['data']['isJoinSquad']:
                ...
                print(f"{Fore.YELLOW + Style.BRIGHT}\r[ Squad ] : Haven't joined the squad yet. Sizif", end="", flush=True)
                time.sleep(3)
                join_result = join_squad(token, "t.me/sizifart")
                if join_result:
                ...
                if recover_coin_pool(token):
                    ...
                    if collect_info:
                        ...
                        if coin_pool_left_count > 0:
                            ...
                            if collect_result and collect_result.get('code') == 0:
                                collected_amount = collect_result['data']['collectAmount']
                                print(f"{Fore.GREEN + Style.BRIGHT}\r[ Collect ] : Collected successfully {collected_amount} coin", flush=True)
                            else:
        ...
        for detik in range(waktu_tunggu, 0, -1):
            sys.stdout.write(f"\r{Fore.CYAN}Waiting for the next claim time in{Fore.CYAN}{Fore.WHITE}{detik // 60} minute {Fore.WHITE}{detik % 60} second")
            sys.stdout.flush()
            time.sleep(1)
...
def parse_arguments():
    ...
    parser.add_argument('--task', type=str, choices=['y', 'n'], help='Check and Claim Tasks (y/n)')
    parser.add_argument('--multi', type=str, choices=['y', 'n'], help='Multi Value Upgrade(y/n)')
    ...
    if args.task is None:
        task_input = input("Auto Complete task? (y/n): ").strip().lower()
        args.task = task_input if task_input in ['y', 'n'] else 'n'
    
    if args.multi is None:
        multi_input = input("Auto Upgrade Value? (y/n): ").strip().lower()
        args.multi = multi_input if multi_input in ['y', 'n'] else 'n'
    
    if args.fill is None:
        fill_input = input("Auto Upgrade Fill? (y/n): ").strip().lower()
